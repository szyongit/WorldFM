...
async function execute(client, interaction) {
    ...
    if (!channel) {
        if (doc) {
            ...
            const newMessage = await messageChannel.send({ embeds: [controls_1.default.getDefaultEmbed()], components: [controls_1.default.buttons] });
            const lock = (lockChannel === undefined ? doc.lock : lockChannel);
            await databasehandler_1.default.ControlsData.updateOne({ guild: guildId }, { channel: messageChannel.id, message: newMessage.id, lock: lock }, { upsert: true }).exec()
                .then(async () => {
                if (lock) {
                ...
                await controls_1.default.update(client, guildId, undefined, undefined);
    ...
    const message = await newChannel.send({ embeds: [controls_1.default.getDefaultEmbed()], components: [controls_1.default.buttons] });
    if (!message) {
    ...
    await databasehandler_1.default.ControlsData.updateOne({ guild: guildId }, { channel: channel.id, message: message.id, lock: locked }, { upsert: true }).exec()
        ...
        await controls_1.default.update(client, guildId, undefined, undefined);
        if (interaction.replied) {
            await interaction.editReply({ embeds: [replyembed_1.default.build({ title: "Controls are now shown inside of <#" + newChannel.id + ">!" })] })
                .then(message => setTimeout(() => message.delete(), 3000));
        }
        else {
            await interaction.reply({ embeds: [replyembed_1.default.build({ title: "Controls are now shown inside of <#" + newChannel.id + ">!" })] })
                .then(message => setTimeout(() => message.delete(), 3000));
        }
    }).catch(() => {
        if (interaction.replied) {
