...
async function testFetchTime() {
    const result = { world: undefined, europe: undefined };
    try {
        const worldStart = Date.now();
        await (0, node_fetch_1.default)(worldURL, { timeout: 5000 });
        result.world = (Date.now() - worldStart);
    }
    catch { }
    try {
        const europeStart = Date.now();
        await (0, node_fetch_1.default)(europeanURL, { timeout: 5000 });
        result.europe = (Date.now() - europeStart);
    }
    catch { }
    return result;
}
...
async function fetchData(countryName, isoCode, isEuropean, skipExisting, progressCallback) {
    let countryId = 1;
    try {
        const regions = (isEuropean ? await fetchEuropeanRegions(isoCode) : await fetchWorldRegions(isoCode));
        if (!regions)
            return undefined;
        let databaseData = [];
        const result = [];
        ...
        if (doc && doc.regions) {
            doc.regions.forEach((element) => {
                ...
                databaseData.push({ region_name: element.region_name, region_id: element.region_id, stations: stations });
            });
        }
        ...
        if (regions.length <= 0) {
            ...
            const regionImage = await fetchRegionImage(regionName.toLowerCase());
...
async function fetchEuropeanRegions(isoCode) {
    try {
        ...
        for (let i = 0; i < elements.length; i++) {
            ...
            const image = await fetchRegionImage(name.toLowerCase());
...
async function fetchWorldRegions(isoCode) {
    try {
...
async function fetchRegionImage(regionName) {
    try {
        const fetched = await (0, node_fetch_1.default)("https://api.teleport.org/api/cities/?search=" + regionName);
        const jsonData = await fetched.json();
        const geonameIdData = jsonData._embedded['city:search-results'][0]._links['city:item'].href;
        if (!geonameIdData)
            return undefined;
        const fetched2 = await (0, node_fetch_1.default)(geonameIdData);
        const jsonData2 = await fetched2.json();
        const imagesData = jsonData2._links['city:urban_area'].href + "images";
        if (!imagesData)
            return undefined;
        const fetched3 = await (0, node_fetch_1.default)(imagesData);
        const jsonData3 = await fetched3.json();
        if (jsonData3.photos.length < 1)
            return undefined;
        return jsonData3.photos[0].image.mobile; //alt: instead of .mobile you can to .web
    }
    catch {
        return undefined;
    }
